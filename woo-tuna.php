			...
			"PaymentData" => [
				...
				"PaymentMethods" => [
					[
            			...
            			"Amount" => round(floatval($customer_order->get_total()), 2),
				...
				if (strval($response->code) == "1" && ($response->status == "C" || $response->status == "P")) {
					...
					if (sanitize_text_field($_POST["tuna_is_pix_payment"]) == "true") {
						if ($response->methods != null && $response->methods[0]->pixInfo != null) {
							$pixImageInfo = $response->methods[0]->pixInfo->qrImage;
							if (stripos($pixImageInfo,'data:image') === false)
                            {
                                $pixImageInfo = 'data:image/png;base64,'.$pixImageInfo;
                            }
							$customer_order->add_order_note("PixImage:".$pixImageInfo);
